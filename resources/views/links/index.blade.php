                            ...
                            <tbody>
                                @foreach($links as $link)
                                    <tr>
                                        ...
                                        <td class="d-flex">
                                            <a href="{{ route('link.up', ['id' => $link->id, 'page'=> request('page')]) }}" class="btn-green text-6 ml-1 ml-sm-2" data-toggle="tooltip" data-original-title="{{ __('messages.table_moveup_action_label') }}">
                                                <i class="fas fa-arrow-circle-up d-none d-sm-block"></i>
                                                <i class="fas fa-arrow-circle-up fa-sm d-sm-none"></i>
                                            </a>
                                            <a href="{{ route('link.down', ['id' => $link->id, 'page'=> request('page')]) }}" class="btn-green text-6 ml-1 ml-sm-2" data-toggle="tooltip" data-original-title="{{ __('messages.table_movedown_action_label') }}">
                                                <i class="fas fa-arrow-circle-down d-none d-sm-block"></i>
                                                <i class="fas fa-arrow-circle-down fa-sm d-sm-none"></i>
                                            </a>
                                            <a href="{{ route('link.edit', ['id' => $link->id]) }}" class="btn-green text-6 ml-1 ml-sm-2" data-toggle="tooltip" data-original-title="{{ __('messages.table_edit_action_label') }}">
                                                <i class="fas fa-pen d-none d-sm-block"></i>
                                                <i class="fas fa-pen fa-sm d-sm-none"></i>
                                            </a>
                                            <form id="form-destroy-slot" method="POST" action="{{ route('link.destroy', ['id' => $link->id, 'page'=> request('page')]) }}">
                                                @csrf
                                                {{ method_field('DELETE') }}
                                            </form>
                                            <a href="{{ route('link.destroy', ['id' => $link->id, 'page'=> request('page')]) }}" class="btn-green text-6 ml-1 ml-sm-2 submit-previous-form" data-toggle="tooltip" data-original-title="{{ __('messages.table_destroy_action_label') }}">
